...
from warp.sim.collide import (count_body_cloth_intersections,
...
class Cloth:
    ...
    def build_stage(self, config):
        ...
        if self.enable_body_smoothing:
            ...
            self.body_smoothing_frames = [smoothing_recover_start_frame +
                                          smoothing_frame_gap_between_steps*i for i in range(smoothing_num_steps + 1)]
        ...
        cloth_reference_labels, body_parts = assign.panel_assignment(
            cloth_seg_dict, cloth_vertices, cloth_indices, wp.transform(
                cloth_pos, cloth_rot),
            body_seg, body_vertices, body_indices, wp.transform(
                body_pos, body_rot),
            device=self.device,
                ...
                if attach_label == 'lower_interface':
                    ...
                    if '_waist_level' in body_dict:
                        ...
                        waist_level = body_dict['height'] - \
                            body_dict['head_l'] - body_dict['waist_line']
                    ...
                    # Attach under arm
                    level = body_dict['height'] - \
                        body_dict['head_l'] - body_dict['armscye_depth']
                    builder.add_attachment(
                        constaint_verts,
                        wp.vec3(0, level, 0),
                        wp.vec3(0., 1., 0.),    # Vertical attachment
    ...
    def _sim_frame_with_substeps(self):
        ...
        for s in range(self.sim_substeps):
            ...
            (self.state_0, self.state_1) = (
                self.state_1, self.state_0)  # swap prev, new state
    ...
    def _build_vert_connectivity(self, vertices, indices):
        ...
        for face_id in range(int(len(indices) / 3)):
            v1, v2, v3 = indices[face_id*3 +
                                 0], indices[face_id*3 + 1], indices[face_id*3 + 2]
